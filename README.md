...
### v1.13.2
- **`/review` split in two.** The charts and the tables now live on separate pages. **Conditions Reviewer** (`/review`) keeps the region and review-period pickers with the days-per-rating histogram, the days-between-fun+-swells histogram, and the daily peak energy and period charts. **Seasonal Analysis** (`/seasons`, new) takes the four season-by-year tables with a region picker only; its Solid and Firing columns are combined into one **Solid or Firing** column, and the drought column reports the **average (median)** gap length in days (`mean_drought` joins `median_drought` in `/api/review/seasons`). Both pages close with the fun+ swell / fun+ day definition paragraph; neither shows the full rating scale any more. Both share the remembered region (`cs_review_region`) and `?region=<buoy_id>` deep links.
- **Season droughts count to the edges.** On the Seasonal Analysis tables a drought is no longer only a gap that closes inside the season: each season also counts the run from its first observed day to its first fun+ swell and from its last fun+ swell to its last observed day (today, while the season is in progress), and a season with no fun+ day at all is one drought spanning its observed days. A drought straddling two seasons is therefore counted in both, each for the part that fell inside it. The Conditions Reviewer's gap histogram applies the same rule over the review period (`droughtsOf`), and its caption now leads with the average. The season tables also overlay the same live tail `/api/review` uses, so the season in progress agrees with the Reviewer down to today's row.
- **Reviewer: the season in progress is a preset.** The review-period picker lists the current season first (`Summer '26 (so far)`, its start → today) ahead of the completed seasons.
- **Bigger, persistent header.** Both pages get a taller header (54 px, 46 px on phones) with the logo and page name at 18 px; they never leave. Once the region selector scrolls under the header, the region rides up into it — right side on desktop, its own row under the title on phones — instead of replacing the page name.
- **One card per thing.** The Reviewer's four charts and the Seasonal Analysis's four season tables are each their own card; the region is no longer repeated at the top of a card (the Reviewer's fun+ count for the period moved up beside the date range), and the Seasonal Analysis's descriptor lines under the region picker and above the tables are gone.
- **Descriptor folds away.** The fun+ / drought definition paragraph on both pages is minimized by default behind a "How fun+ days and droughts are counted" toggle line.
- **Seasonal Analysis tables at narrower widths.** The drought header is the compact `Drought d / avg (med)` at every width, tables use fixed layout everywhere, and the season cards drop from four across to two below 1400 px, so mid-width desktop windows no longer overflow. The Solid/Firing column's wash is a darker amber (the scheme's SOLID colour pulled 55 % toward black, at a stronger opacity ramp).
- **Reviewer links back.** Under the descriptor, "Comparison of key metrics across different seasons over the years can be found in the Seasonal Analysis →" (region-linked).
- **Reaching them.** The dashboard's More modal lists ▦ SEASONAL ANALYSIS first, then ▤ CONDITIONS REVIEWER. The regional view's fun+ summary row gains a second text button, ▦ REVIEW SEASONAL ANALYSIS, beside ▤ CONDITIONS REVIEWER on their own line under the tally and drought text. The Seasonal Analysis page links to the Conditions Reviewer at the bottom ("More detailed historical analysis … can be found in the Conditions Reviewer →") and in its More modal; the Reviewer's More modal links back.
